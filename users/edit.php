...
function edit ($id_post) {
  ...
  if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
    ...
    $user['avatar'] = '/php_mysql'.substr($target, 2);
  }
  ...
  // $user => 修改过后的信息
  // TODO: 将数据更新回数据库
  // UPDATE users SET avatar='/php_mysql/users/assets/img/icon-40.png' where id%2=1;
  // $queryy = mysqli_query($conn, "insert into users values (null, '{$name}', {$gender}, '{$birthday}', '{$avatar}');");
  ...
  $queryy = mysqli_query($connection, "update users set avatar='{$user['avatar']}',name='{$user['name']}',gender={$user['gender']},birthday='{$user['birthday']}' where id={$id_number};");

  if (!$queryy) {
    $GLOBALS['error_message'] = '更新过程失败';
    return;
  }
